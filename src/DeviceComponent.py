...
class DeviceComponent(DeviceComponentBase):
    ...
    def __init__(self, *a, **k):
        ...
        for device_name, current_banks in self._device_banks.iteritems():
            # log_message("################## device_name : %s  bank_len: %d" % (device_name, len(current_banks)))
            # for current_bank in current_banks:
            #    log_message("    #### bank : %s" % current_bank)
            # log_message("    #### device_name in self._device_best_banks : %s" % (device_name in self._device_best_banks))
            ...
            if len(current_banks) > 1:
                if not device_name in self._device_best_banks:
                    raise AssertionError, "Could not find best-of-banks for '%s'" % device_name
                if not device_name in self._device_bank_names:
                ...
                new_banks[device_name] = self._device_best_banks[device_name] + current_banks
            else:
                new_bank_names[device_name] = (BOB_BANK_NAME,)
    ...
    def _update_device_bank_buttons(self):
        if self.is_enabled():
            ...
            for index, button in enumerate(self._bank_buttons or []):
                if button:
                    value_to_send = 'DefaultButton.Disabled'
...
# okay decompyling /Applications/Ableton Live 9 Suite.app/Contents/App-Resources/MIDI Remote Scripts/Launchkey_MK2/DeviceComponent.pyc 
