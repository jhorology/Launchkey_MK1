...
from .ControlElementUtils import make_button, make_pad_button, make_encoder, make_slider
...
from .Log import log_message, set_log_instance
import consts

class Launchkey_MK1(OptimizedControlSurface):
    identity_request_delay = 0.5
    encoder_in_control_double_click_time = 0.3
    ...
    def _create_controls(self):
        ...
        self._top_pad_row = ButtonMatrixElement(rows=[[ make_pad_button(identifier, name='Pad_0_%d' % (index,), skin=self._skin) for index, identifier in enumerate(xrange(96, 104)) ]])
        self._bottom_pad_row_raw = [ make_pad_button(identifier, name='Pad_1_%d' % (index,), skin=self._skin) for index, identifier in enumerate(xrange(112, 120)) ]
        self._bottom_pad_row = ButtonMatrixElement(rows=[self._bottom_pad_row_raw])
        self._top_launch_button = make_pad_button(104, name='Scene_Launch_Button', skin=self._skin)
        self._bottom_launch_button = make_pad_button(120, name='Stop_All_Clips_Button', skin=self._skin)
        ...
        self._encoder_incontrol_button.add_value_listener(self._update_encoder_mode)
    ...
    def _create_mode_selector(self):
        ...
        self._mode_selector.add_mode('mode_selection', [partial(self._toggle_in_control, True), mode_selection, device_navigation])
    ...
    def handle_sysex(self, midi_bytes):
        if self._is_identity_response(midi_bytes):
            ...
            super(Launchkey_MK1, self).handle_sysex(midi_bytes)
    ...
    def _is_identity_response_valid(self, product_id_bytes):
        return product_id_bytes[:4] == consts.PRODUCT_ID_BYTE_PREFIX and product_id_bytes[6] in consts.PRODUCT_ID_BYTES

    def _set_model_type(self, product_id_bytes):
        self._is_25_key_model = product_id_bytes[6] == consts.LAUNCHKEY_25_ID_BYTE
    ...
    def on_identified(self):
        ...
        self._send_midi(consts.LED_FLASHING_ON)
        with self.component_guard():
            self._create_mixer()
    ...
    @subject_slot('value')
    def _update_encoder_mode(self, value):
        current_mode = self._mode_selector.selected_mode
        if value == 0:
            self._enable_toggle_encoder_mode()
        else:
            if self._encoder_mode_toggle_enabled:
                if current_mode == 'session_mode':
                    self._mode_selector.selected_mode = 'mode_selection'
                else:
                    self._mode_selector.selected_mode = 'session_mode'
            else:
                self._mode_selector.selected_mode = 'session_mode'
            self._disable_toggle_encoder_mode()
            
    def _enable_toggle_encoder_mode(self):
        self._encoder_mode_toggle_enabled = True
        self._encoder_mode_task.restart()
        
    def _disable_toggle_encoder_mode(self):
        self._encoder_mode_task.kill()
        self._encoder_mode_toggle_enabled = False
        
    def disconnect(self):
        self._send_midi(consts.LED_FLASHING_OFF)
